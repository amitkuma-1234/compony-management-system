  ...
  if (userJson) {
    try {
      ...
      if (sidebarRole) {
        ...
        const roleColors = {
          admin: { bg: '#f59e0b', color: '#000', label: 'Admin' },
          hr: { bg: '#a855f7', color: '#fff', label: 'HR' },
          manager: { bg: '#06b6d4', color: '#fff', label: 'Manager' },
          employee: { bg: '#22c55e', color: '#fff', label: 'Staff' }
        };
        const rc = roleColors[roleKey] || roleColors.employee;
        const badge = document.createElement('span');
        badge.className = 'role-badge';
        badge.textContent = rc.label;
        badge.style.cssText = `display:inline-block;margin-top:4px;padding:1px 8px;border-radius:20px;font-size:9px;font-weight:800;background:${rc.bg};color:${rc.color};letter-spacing:0.4px;text-transform:uppercase;`;
        // Insert after name or role
        const infoContainer = document.querySelector('.sidebar-user .user-info');
